...
module mod_minimize
  ...
  public :: minimize_spec, minimize, minimize_mpi
...
contains
  ...
  ! Minimize algorithn for a cube with regularization, version : parallelize f_g_cube_fast with MPI
  subroutine minimize_mpi(n, m, x, lb, ub, cube, n_gauss, dim_v, dim_y, dim_x, lambda_amp, lambda_mu, lambda_sig, &
    ...
    do while(task(1:2).eq.'FG'.or. task.eq.'NEW_X' .or. task.eq.'START') 
       ...
       if (task(1:2) .eq. 'FG') then          
          ...
          if (lym .eqv. .true.) then
             ...
             call f_g_cube_fast_mpi(f, g, cube, x, dim_v, dim_y, dim_x, n_gauss, kernel, lambda_amp, lambda_mu, lambda_sig, &
