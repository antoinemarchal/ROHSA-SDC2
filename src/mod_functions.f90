...
module mod_functions
  ...
  public :: mean_array, mean_map, dim2nside, dim_data2dim_cube, reshape_up, reshape_down, go_up_level, init_spectrum, &
       upgrade, update, set_stdmap, set_stdmap_full, std_spectrum, mean_spectrum, max_spectrum, init_grid_params, init_new_gauss, &
       reshape_noise_up
...
contains
  ...
  subroutine set_stdmap_full(std_map, cube) !fixme allocate deallocate
    !! Compute the STD map of a 3D array
    implicit none
    ...
    deallocate(line)
  end subroutine set_stdmap_full
