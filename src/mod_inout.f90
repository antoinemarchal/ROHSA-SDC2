...
module mod_inout
  ...
  public :: read_cat, read_cube, read_output, read_map, read_array, read_parameters, save_process

contains  
  
  subroutine read_parameters(filename_parameters, filename, filename_cat, fileout, timeout, filename_noise, &
    ...
    real(xp), intent(inout) :: norm_cube
    ...
    character(len=512), intent(inout) :: filename_cat
    ...
    namelist /user_parameters/ filename, filename_cat, fileout, timeout, filename_noise, filename_init_spec, &
  ...
  subroutine read_cat(filename,x,y,f,c,nl)
    implicit none

    character(len=512), intent(in) :: filename
    integer, intent(in) :: nl

    integer, intent(inout), dimension(:), allocatable :: x,y,f
    real(xp), intent(inout), dimension(:), allocatable :: c
    ...
    do i=1,nl
       read(11,fmt=*) x(i), y(i), f(i) !ra, dec, freq, confiance
    enddo
